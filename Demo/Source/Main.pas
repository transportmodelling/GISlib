...
uses
  ...
  Vcl.ExtCtrls, GIS, GIS.Shapes, GIS.Shapes.ESRI, GIS.Shapes.GeoJSON, GIS.Render.Shapes, GIS.Render.PixelConv, GIS.Render.PixelConv.Cartesian;
...
type
  ...
  private
    ...
    Var
      ...
      MouseDown: Boolean;
      PixelConverter: TCartesianPixelConverter;
      ShapesLayer: TShapesLayer;
...
begin
  ...
  FileOpen.Dialog.InitialDir := ExpandFileName(ExtractFilePath(Application.ExeName) + '..\Data');
  ...
  PixelConverter := TCartesianPixelConverter.Create;
...
begin
  ...
  if PixelConverter.Initialized then
  begin
...
begin
  ...
  begin
    ...
    case ZoomStyle of
      zsMove:
        begin
          PixelConverter.PanMap(MousePosition.X-StartPosition.X,MousePosition.Y-StartPosition.Y);
      ...
      zsZoomIn:
        begin
          if (MousePosition.X=StartPosition.X) and (MousePosition.Y=StartPosition.Y) then
            PixelConverter.ZoomIn(MousePosition)
          else
            begin
              ...
              PixelConverter.ZoomIn(TRectF.Create(Left,Top,Right,Bottom));
            end;
          ShapesImageStatus := isViewportChanged;
          PaintBox.Invalidate;
        end;
      zsZoomOut:
        begin
          ...
          PixelConverter.ZoomOut(TPointF.Create(CenterX,CenterY));
          ShapesImageStatus := isViewportChanged;
          PaintBox.Invalidate;
        end;
    end;
...
begin
  ...
  try
    ...
    var Reader: TGISShapesFormat;
    if SameText(ExtractFileExt(FileName),'.geojson') then
      Reader := TGeoJSONReader
    else
      Reader := TESRIShapeFileReader;
    ESRIShapesFile.Read(FileName,Reader);
    if ESRIShapesFile.ShapeCount(stPoint) = 0 then PixelConverter.Margin := 0 else PixelConverter.Margin := ESRIShapesFile.PointRenderSize;
    // Show layer
    ShapesLayer.Free;
